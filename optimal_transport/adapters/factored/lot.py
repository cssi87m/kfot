...
class LOT(OT):
    def __init__(
        ...
        n_source_anchors, n_target_anchors, div_term=1e-10,
        epsilon=1, epsilon_z=1, intensity=[10, 10, 10], floyditer=50,
        tolratio=1e-7, norm=2, random_state=None, **kwargs
    ):
    ...
    def transport(self, source, target, **kwargs) -> np.ndarray:
        n = source.shape[0]
        m = target.shape[0]
        Cx_lot = self.Pi_[0].T.dot(source) / (self.Pi_[0].T.dot(np.ones([n, 1])) + 10 ** -20)
        Cy_lot = self.Pi_[1].dot(target) / (self.Pi_[1].dot(np.ones([m, 1])) + 10 ** -20)
        transported = source + np.dot(
            np.dot(
                self.Pi_[0] / np.sum(self.Pi_[0], axis=1).reshape([n, 1]),
                self.Pi_[2] / np.sum(self.Pi_[2], axis=1).reshape([self.n_source_anchors, 1])
            ),
            Cy_lot) - np.dot(self.Pi_[0] / np.sum(self.Pi_[0], axis=1).reshape([n, 1]), Cx_lot)
        return transported
    ...
    def robust_transport(self, source, target, threshold=0.8, decay=0) -> np.ndarray:
        ...
        maxPz = np.max(self.Pi_[2], axis=1)
        Pz_robust = self.Pi_[2].copy()

        for i in range(0, self.n_source_anchors):
            for j in range(0, self.n_target_anchors):
                if self.Pi_[2][i, j] < maxPz[i] * threshold:
                    Pz_robust[i, j] = self.Pi_[2][i, j] * decay
        Pz_robust = Pz_robust / np.sum(Pz_robust, axis=1).reshape([self.n_source_anchors, 1]) * \
                    np.sum(self.Pi_[2], axis=1).reshape([self.n_source_anchors, 1])
